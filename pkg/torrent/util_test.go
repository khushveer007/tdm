...
func TestGetMetainfo(t *testing.T) {
	// Returns bytes that are not a valid .torrent, forcing metainfo.Load to error.
	...
	tests := []struct {
		...
		{"http.Get error", "http://[::1]:-1", true},
		{"invalid torrent payload", bad.URL, true},
	}
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			mi, err := torrent.GetMetainfo(tc.url)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("expected error, got nil")
				}
				if mi != nil {
					t.Fatalf("expected nil metainfo on error, got non-nil")
				}
				return
			}
			if err != nil {
			...
			if mi == nil {
				t.Fatalf("expected metainfo, got nil")
			}
		})
	}
}
