...
func TestFactoryCreate(t *testing.T) {
    t.Run("successful creation", func(t *testing.T) {
        ...
        proto, err := f.Create(context.Background(), "http://example.com",
    ...
    t.Run("invalid URL", func(t *testing.T) {
        f := NewDefaultFactory(FactoryOptions{})
        _, err := f.Create(context.Background(), "invalid-url")
        assert.ErrorIs(t, err, ErrInvalidURL)
    })

    t.Run("unsupported protocol", func(t *testing.T) {
        f := NewDefaultFactory(FactoryOptions{})
        _, err := f.Create(context.Background(), "unsupported://example.com")
    ...
    t.Run("initialization error", func(t *testing.T) {
        ...
        _, err = f.Create(context.Background(), "http://example.com")
...
func TestFactoryConcurrency(t *testing.T) {
    t.Run("concurrent registration and creation", func(t *testing.T) {
        ...
        for i := 0; i < 10; i++ {
            ...
            go func() {
                defer wg.Done()
                _, _ = f.Create(context.Background(), "http://example.com")
            }()
        }
